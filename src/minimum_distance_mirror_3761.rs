...
use std::collections::HashMap;
    ...
    pub fn min_mirror_pair_distance(nums: Vec<i32>) -> i32 {
        let mut last_position_by_value: HashMap<i32, usize> = HashMap::with_capacity(nums.len());
        nums.into_iter().enumerate().filter_map(|(position, value)| {
            let mirror = mirror(value);
            let prev_position_option = last_position_by_value.get(&value).copied();
            last_position_by_value.insert(mirror, position);
